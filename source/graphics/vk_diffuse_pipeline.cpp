...
namespace INJECTOR_NAMESPACE
{
		...
		const vk::Extent2D& surfaceExtent)
	{
		...
		if (resultValue.result != vk::Result::eSuccess)
			throw std::runtime_error("Failed to create Vulkan diffuse pipeline");
	...
	std::vector<VkBufferHandle> VkDiffusePipeline::createUniformBuffers(
		VmaAllocator allocator,
		uint32_t imageCount)
	{
		auto uniformBuffers = std::vector<VkBufferHandle>(imageCount);
