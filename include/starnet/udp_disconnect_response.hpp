...
namespace starnet
{
	struct UdpDisconnectResponse final : inject::UdpRequestResponse
	{
		enum Reason : uint8_t
		{
			Requested,
		...
		UdpDisconnectResponse(const uint8_t _reason) :
			reason(_reason)
		{}
		UdpDisconnectResponse(const std::vector<uint8_t>& buffer, const size_t count)
		{
